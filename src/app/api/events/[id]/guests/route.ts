...
import { sendInvitationEmail, sendConfirmationEmail } from '@/lib/email'
...
export async function POST(req: NextRequest, { params }: Params) {
  const { id } = await params
  const { name, email, phone, direct } = await req.json()
  ...
  if (direct) {
    await sendConfirmationEmail(guest, event).catch(() => {})
  } else {
